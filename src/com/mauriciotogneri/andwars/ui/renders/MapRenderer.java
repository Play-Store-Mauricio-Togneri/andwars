...
public class MapRenderer extends SurfaceView implements SurfaceHolder.Callback
{
	...
	@NonNull
	private final Paint adjencyPaint;
	
	@NonNull
	private final List<Point> background = new ArrayList<Point>();

	protected static int BLOCK_WIDTH = 0;
	protected static int BLOCK_HEIGHT = 0;
	protected static int FONT_SIZE = 0;
	...
	public MapRenderer(@NonNull Context context, MapListener mapListener)
	{
		...
		this.adjencyPaint = new Paint();
		this.adjencyPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
		this.adjencyPaint.setColor(Color.GRAY);
		this.adjencyPaint.setStrokeWidth(5);
		...
		try
		{
			...
			if (canvas != null)
			{
				synchronized (surfaceHolder)
				{
					...
					drawAdjacencies(cells, this.adjencyPaint, canvas);
	...
	private void drawAdjacencies(List<Cell> cells, Paint adjencyPaint, Canvas canvas)
	{
		for (Cell cell : cells)
		{
			drawAdjacency(cell, adjencyPaint, canvas);
		}
	}
	
	private void drawAdjacency(Cell cell, Paint adjencyPaint, Canvas canvas)
	{
		...
		for (Cell adjacent : cell.getAdjacents())
		{
			...
			canvas.drawLine(cellCenter.x, cellCenter.y, adjacentCenter.x, adjacentCenter.y, adjencyPaint);
		}
	}
